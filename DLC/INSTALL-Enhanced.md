# ExtendedLSC Glass DLC — Enhanced (`dlc_elsc`)
...
4. Drop that `vehicles.rpf` into `dlc_elsc/x64/levels/gta5/` (replacing the PUT_..._HERE.txt marker).

## Install (no base files overwritten)
The shell (`content.xml`, `setup2.xml`) is already staged at:
`...\Grand Theft Auto V Enhanced\mods\update\x64\dlcpacks\dlc_elsc\`
...
## Uninstall
- Delete the `dlc_elsc` folder + remove the dlclist line. Base game untouched.
